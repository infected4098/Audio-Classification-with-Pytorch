...
X_data = np.load(X_mel_path) #(batch_size, 44, 128)
X_data = torch.Tensor(X_data).permute(0, 2, 1).numpy() #(batch_size, 128, 44)
y_data = np.load(y_mel_path) #(batch_size, 14)
...
model_3_mel = ResNet(BasicBlock, [2, 1, 1, 1]).to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model_3_mel.parameters(), lr = CONFIG["lr"])
"""
loss_history = []
val_loss_history = []
init_loss = 999999
        ...
        if i % 500 == 499:
            ...
            with torch.no_grad():
                ...
                for k, (val_inputs, val_labels) in enumerate(test_loader):
                    val_output = model_3_mel(val_inputs)
            ...
            if val_loss < init_loss:
                torch.save(model_3_mel, os.path.join(save_path, 'resnet_mel_1.pt'))
...
np.save("C:/Users/infected4098/Desktop/LYJ/Audio Clf with Pytorch/resnet_mel_valloss.npy", val_loss_history)
np.save("C:/Users/infected4098/Desktop/LYJ/Audio Clf with Pytorch/resnet_mel_loss.npy", loss_history)
#torch.save(model_1.state_dict(), "C:/Users/infected4098/Desktop/LYJ/Audio Clf with Pytorch/cnn_mfcc.pt")
"""
resnet_mel = torch.load(os.path.join(save_path, 'resnet_mel_1.pt'))
