...
newcnn_mfcc = NewCNN_res(32, (13, 49)).to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(newcnn_mfcc.parameters(), lr = CONFIG["lr"])
        ...
        if i % 500 == 499:
            ...
            with torch.no_grad():
                ...
                for k, (val_inputs, val_labels) in enumerate(test_loader):
                    val_output = newcnn_mfcc(val_inputs)
            ...
            if val_loss < init_loss:
                torch.save(newcnn_mfcc, os.path.join(save_path, 'newcnnres_mfcc.pt'))
...
np.save("C:/Users/infected4098/Desktop/LYJ/Audio Clf with Pytorch/newcnnres_mfcc_valloss.npy", val_loss_history)
np.save("C:/Users/infected4098/Desktop/LYJ/Audio Clf with Pytorch/newcnnres_mfcc_loss.npy", loss_history)
#torch.save(model_1.state_dict(), "C:/Users/infected4098/Desktop/LYJ/Audio Clf with Pytorch/cnn_mfcc.pt")


resnet_mfcc = torch.load(os.path.join(save_path, 'newcnnres_mfcc.pt'))
